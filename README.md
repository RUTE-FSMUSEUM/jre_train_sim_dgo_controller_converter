...
* ソースコードを書き換えることで、入力キーマッピングの変更や JC-PS101 以外のゲームパッドコンバータに対応可

## リリース
### ダウンロード
* 自分の環境では動いたものは[こちら](https://fsmuseum.net/souvenir/trainsimutil/DgoConverter.0.4.2.zip)または[Release](https://github.com/RUTE-FSMUSEUM/jre_train_sim_dgo_controller_converter/releases/tag/v0.4.2)で公開しています
### 最新版の変更点
* PC に接続されているコントローラをリストアップしました
* リストアップされているコントローラのうち、任意の一つを DgoConverter に接続できるようになりました
* 以上の2点の機能追加によって、より多くの方の環境で DgoConverter が正しく動作するようになりました
* UI を一部変更しました
* 暫定リリースからβ版相当に移行しました
...
4. JR東日本トレインシミュレータを起動
5. 京浜東北線（ワンハンドル）の場合は、Target: を **JRE SIM ONE HANDLE** に、八高線（ツーハンドル）の場合は、Target: を **JRE SIM TWO HANDLE** に設定
6. ローディングが終了したら、コントローラを**非常の位置**に入れる
7. JR東日本トレインシミュレータ上で非常ブレーキが有効になったことを確認
8. DgoConverter.exe 上の Est. mascon state が EB になったことを確認
...
4. Device: から **JC-PS101U** を選択
5. HMMSIM METRO を起動
6. Target: から **HMMSIM METRO** に設定
7. コントローラを**通常ブレーキ8の位置**に入れる
8. ローディングが終了したら、HMMSIM METRO 上で非常ブレーキが有効になっていることを確認
9. コントローラを**通常ブレーキ8の位置**に入れ、HMMSIM METRO 上で**非常ブレーキが解除された**ことを確認（ベルが鳴るはずです）
10. DgoConverter.exe 上の Est. mascon state が B7 になったことを確認
...
* に登場する車両は、非常ブレーキ1段、通常ブレーキ7段、力行（ノッチ）4段です
...
## 注意点
* コントローラの状態は、0.03秒に1回読み取られます
* 0.03秒以内に2回以上マスコンを操作すると、DgoConverter.exe 上でマスコン状態が正しく推定できず、正しく動作しなくなる可能性があります
