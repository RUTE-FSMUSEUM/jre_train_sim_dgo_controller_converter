...
* 一部コントローラでは、コントローラ上のボタンで十字キー操作などが可能
...
### 最新版の変更点
* ZUIKI 製コントローラ上のボタンより十字キー操作、決定（Enter）、戻る（Esc）操作が行えるようになりました
* JR東日本トレインシミュレータのすべてのキーアサイメント（EBリセットや連絡ブザー、勾配起動など）に対応可能となりました
* ソフトウェアからの仮想キー入力を改善しました
* UI を一部変更しました
...
6. **-24** となっている場合はEvent Trigger Map:から **JC-PS101U** を選択し、**-8** となっている場合は **JC-PS101U - X Axis = -8 at P5** を選択
...
### JR東日本トレインシミュレータ｜詳細なボタンのデフォルトアサイメント
| ボタン名称 | 割り当てされているコマンド | キーボード上での割り当て
| ----- | ----- | ----- |
| A | 警笛（2段目） | Backspace |
| B | 連絡ブザー | B |
| C | 警笛（1段目）・決定 | Enter |
...
4. Device: から **JC-PS101U** を選択
5. コントローラを P5 に入れ、Dgoconverter 画面の X Axis（Dgo controller state の左上の項目）の値を確認する
6. **-24**となっている場合は Event Trigger Map: から **JC-PS101U** を選択し、**-8**となっている場合は **JC-PS101U - X Axis = -8 at P5** を選択
7. HMMSIM METRO を起動
...
* ノッチがニュートラル（切）まで戻らなくなった場合は、コントローラを切の位置にした上で、HMMSIM METRO の電車がニュートラル状態になるまで、**C ボタンを長押ししてください**
* 以上を踏まえまして、HMMSIM METRO ではより一層丁寧でゆっくりとしたコントローラ操作を強く推奨いたします

### HMMSIM METRO｜詳細なボタンのデフォルトアサイメント
| ボタン名称 | 割り当てされているコマンド | キーボード上での割り当て
| ----- | ----- | ----- |
| A | 警笛 | Backspace |
| B | （割り当てなし） | -- |
...
| B | 戻る | Esc |
| Y | EBリセットスイッチ | E |
| △ | レバーサ（逆転器） 前・移動（上） | 十字キー↑ |
| ▷ | 移動（右） | 十字キー→ |
| ▽ | レバーサ（逆転器） 後・移動（下） | 十字キー↓ |
| ◁ | 移動（左） | 十字キー← |
| + | 定速/抑速（抑速2）スイッチ | W |
| - | 抑速1スイッチ | D |
| R | ATS確認 | Space |
...
| ZR | 連絡ブザー | B |
| ZL | （割り当てなし） | -- |
* ZLは、EBと区別がつかない（どちらも `Buttons: 06` となる）ため、原則使用不可と考えています
* 希望があれば、ATS関連に重点を置いたアサイメント、自動運転（TASC、インチングボタン）に重点を置いたアサイメントを追加予定です
